...
enum FbxWriter {
    ...
    /// into the file itself (a Video node with base64 content plus a Texture
    /// node connected to the material's diffuse slot), so the result is one
    ...
    static func writeMesh(
        ...
        if let texture {
            ...
            // An embedded texture is a Video node carrying the file's bytes as
            // base64 text, a Texture node that refers to it, and a connection
            // from the material's diffuse slot to the texture. This is the
            // layout the FBX SDK itself writes; Blender and 3ds Max read both
            // sides of it.
            objectsChildren.append(Node(
                ...
                children: [
                    ...
                    Node("Content", props: [P.str(Data(texture.data).base64EncodedString())]),
                ]
            ))
            objectsChildren.append(Node(
                "Texture",
