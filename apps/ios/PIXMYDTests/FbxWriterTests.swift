...
final class FbxWriterTests: XCTestCase {
    ...
    func testTextureIsEmbeddedAndConnected() throws {
        ...
        // The Video node's Content is the PNG's bytes as base64 text.
        let content = try XCTUnwrap(FbxWriter.findNode(nodes, named: "Content")?.props[0] as? String)
        let decoded = try XCTUnwrap(Data(base64Encoded: content))
        XCTAssertEqual(
            [UInt8](decoded.prefix(8)),
